<?php
	...
	class Configuration {
		...
		public static function SetTpl() {
			Configuration::$tpl = new AdminTemplate(Configuration::$content->template());
		}
	}

	function IncludeLocation($loc, $orientation) {
		$loc = new AdminLocation($loc);
	...
	function SendError($errNo = 0, $errMsg = "Unknown error has occurred", $pid = 0){
		if (isset($_REQUEST['pid']))
		  $pid = $_REQUEST['pid'];
		$url = "/admin/?error=true&errNo=".$errNo."&errMsg=".$errMsg."&pid=".$pid;
		header("Location: $url");
		die();
	}
