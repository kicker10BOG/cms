<?php
	...
	$tpl_leftMenu = 0;
	$tpl_rightMenu = 0;
	$tpl_bannerurl = $home_dir."templates/default/images/banners/JasonLBogle-2admin.png";

	function InsertLocation($locID, $locName) {
		$userStatus = GLB::get('userStatus');
		$query = "SELECT * from ".Configuration::sitePrefix."adminMenus WHERE publish=1 AND location=$locID ORDER BY `order` ASC";
		...
		while ($row = $result->fetch_array(MYSQLI_ASSOC)){
			if ($userStatus >= $row['securelevel']) {
				$query2 = "SELECT * FROM ".Configuration::sitePrefix."adminMenu_items WHERE menu=".$row['id']." ORDER BY `order` ASC";
				    ...
				    if ($row['type'] == 'horizontal') {
						...
						while ($row2 = $result2->fetch_array()) {
							if ($row2['publish'] == 1 && $userStatus >= $row2['securelevel']) {
							    $query3 = "SELECT title, alias, status  FROM ".Configuration::sitePrefix."adminPages WHERE id=".$row2['pid'];
